...
**No opt-out.** Embedded-tweet resolution runs unconditionally — the same posture standalone X extraction (`extraction/x.py`) already has, and deliberately not tied to `ANALECTA_DISABLE_TIER2`: that flag now gates Chromium rendering only. It briefly gated this resolver too (2026-07-23, same day introduced), reasoning that both are network-calling enhancements on top of bare Tier 1; reversed the same day once the flag started being used to test whether Tier 1 alone — resolver included — can cover extraction without Chromium at all, which requires the resolver to run precisely when the flag is set. Reading any third-party article with an embedded tweet contacts `cdn.syndication.twimg.com` with no way to suppress it short of not reading that article. Flagged here as a gap worth a future opt-out, not resolved by this change.
