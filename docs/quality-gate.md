...
8. `pnpm --filter frontend check` — SvelteKit type-checking (`svelte-kit sync && svelte-check`)
9. `pnpm --filter frontend build` — Vite production build
...
### svelte-check: accepted warnings

`svelte-check` must report **0 errors**. Three warnings are accepted as tooling
limitations and must not be fixed by adding wrong semantic roles:

| File | Warning code | Reason |
|------|-------------|--------|
| `src/lib/components/MarkdownEditor.svelte` | `state_referenced_locally` | CodeMirror requires capturing the initial prop value once; `// @svelte-ignore` in `<script>` blocks is not recognised by `svelte-check@4`. |
| `src/routes/viewer/[id]/+page.svelte` | `a11y_no_static_element_interactions` | Click-delegation on a `{@html}` block; `<!-- svelte-ignore -->` is unreliable inside `{:else if}` branches in `svelte-check@4`. |
| `tsconfig.json` | `Cannot find type definition file for 'node'` | Generated by SvelteKit's internal `.svelte-kit/tsconfig.json`; not in our `tsconfig.json`. |

Any warning **outside** these three must be investigated and resolved before considering
a block complete.

### vite build: accepted chunk size hint

`vite build` emits a `(!) Some chunks are larger than 500 kB` hint for the editor route
(`/editor/[id]`). The chunk contains the full CodeMirror 6 ecosystem
(`@codemirror/*`, `@lezer/*`, `@uiw/codemirror-theme-tokyo-night`) which is a monolith
by design. The chunk is already code-split by SvelteKit — it loads only on navigation to
the editor, never on initial app launch. Network latency is irrelevant in a Tauri desktop
app. `build.chunkSizeWarningLimit` is set to `600` in `vite.config.ts` to suppress this
hint. No further action is needed unless a new dependency inflates the chunk past 600 kB.
