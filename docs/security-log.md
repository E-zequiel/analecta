...
- **The pin, not a manifest range:** the load-bearing pin is `package.json`'s `packageManager` field — pnpm's managed-version switch runs that version in-repo regardless of mise. `.mise.toml` deliberately stays `pnpm = "latest"` (maintainer decision, unchanged by this bump); no CI workflow pins pnpm (they invoke it via `jdx/mise-action` + `mise exec -- pnpm`) and no script asserts a pnpm version.
...
- **Lockfile churn observed:** additive only — pnpm 12 prepends a new YAML document to `pnpm-lock.yaml` recording the package-manager dependency itself (`importers: .: packageManagerDependencies: pnpm: {specifier: 12.4.2}`) plus `pnpm@12.4.2` and its 14 `@pnpm/exe.*` platform-binary packages/snapshots; the original lockfile document is byte-identical (zero deleted lines). Install and `install --frozen-lockfile` were both no-ops under 12.4.2 (`Already up to date`; no relink, no store-dir change) and pnpm 12's built-in lockfile supply-chain-policy check passed (569 entries).
