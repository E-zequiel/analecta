...
**Local limitation with `deps_update.py`:** `scripts/deps_update.py` calls `pnpm add <pkg>@<latest> --save-exact` for each outdated Node package, and `pnpm dedupe` afterward (to collapse duplicate resolutions left by a direct-dependency bump — see `_apply_node_package()`). Both commands support `--ignore-scripts` but the script does not pass it to either — `electron`'s postinstall (the only lifecycle script permitted by `allowBuilds`) may therefore run before `socket-audit.sh` can scan the updated lockfile, potentially more than once per run if a batch failure triggers the script's per-package isolation retry. Running the scan immediately after — and not pushing until it is clean — is the correct compensating control. The hard enforcement gate is CI: the `socket` job in `ci.yml` runs with `--ignore-scripts` and its result gates `check-frontend` and `test-backend` via `needs:` (see Control 12).
